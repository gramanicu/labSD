...
int main() {
	...
	for (int i = 0; i < nEdges; i++) {
		in1 >> u >> v;
		g1.addEdge(u, v);/* TODO 1 Initializations */
	...
	if (!bipartite) {
		...
		for (unsigned int i = 0; i < sides[0].size(); i++) {
			out4 << sides[0][i] << " ";
		}
		out4 << endl;
		for (unsigned int i = 0; i < sides[1].size(); i++) {
			out4 << sides[1][i] << " ";
		}
	}
	...
	for (int i = 0; i < nTests; i++) {
		...
		if (gb.hamCycles.size() == 0) {
			...
			for (unsigned int i = 0; i < gb.hamCycles.size(); i++) {
